...
function searchTool(server) {
    return server.tool(TOOLS.search.name, TOOLS.search.description, {
        ...
        const matchingResults = foodDatabase.filter(item => item.name.toLowerCase().includes(queryLower) ||
...
function fetchTool(server) {
    return server.tool(TOOLS.fetch.name, TOOLS.fetch.description, {
