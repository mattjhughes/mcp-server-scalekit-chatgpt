            ...
            try {
                if (!body) {
                    ...
                    pipeline(body, res, (err) => {
                        if (err)
                            logger.warn(`Proxy stream pipeline error [${correlationId}]: ${err.message}`);
                    });
                }
                else {
                ...
                logger.warn(`AUTH GATEWAY [${correlationId}]: Failed to stream body, falling back to buffer: ${e.message}`);
            }
        }
